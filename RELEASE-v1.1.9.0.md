...
- `1.1.9.x` is reserved for final LunarWing v1 hotfixes. The next major release line is v2.
...
- Uploaded SSH keys may still require a daemon restart before they are signable outside the `mt-admin start-tenant` bounce path.
- Production SSH audit logging still uses `NullAuditLogger`.
...
- Inbound XMPP OOB / `aesgcm://` downloads still lack an SSRF guard.
...
- The static web config and some docs still contain stale provider or NEAR-era copy. Active provider routing is the code-backed source of truth.
