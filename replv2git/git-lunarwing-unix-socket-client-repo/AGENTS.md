...
This repository is a small Rust client for the LunarWing Unix socket REPL. The primary package manifest is `Cargo.toml`, and the current implementation lives in `src/lib.rs`. `README.md` only contains a placement note, so keep contributor-facing guidance here. There is also a duplicate `src/Cargo.toml`; if you touch dependency metadata, keep both manifests aligned or remove the duplicate in a dedicated cleanup change.
...
- `cargo run -- ~/.lunarwing/lunarwing.sock` starts the client against an explicit socket path.
