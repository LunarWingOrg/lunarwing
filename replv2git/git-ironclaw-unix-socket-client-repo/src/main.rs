...
async fn main() -> anyhow::Result<()> {
    ...
    println!("LunarWing Unix Socket Client v2");
...
fn resolve_socket_path() -> anyhow::Result<PathBuf> {
    ...
    if let Some(path) = std::env::var_os("LUNARWING_SOCKET") {
...
fn resolve_base_dir() -> PathBuf {
    if let Some(path) = std::env::var_os("LUNARWING_BASE_DIR") {
        if !path.is_empty() {
            return PathBuf::from(path);
        }
    }

    if let Some(path) = std::env::var_os("IRONCLAW_BASE_DIR") {
        if !path.is_empty() {
            return PathBuf::from(path);
        }
    }
