...
New operator tooling (all under `ic/scripts/`, `root`-only, defaulting to safe/dry-run behavior) for **moving and upgrading multi-tenant fleets** without orphaning data or silently resetting operator config. Every script went through multi-agent adversarial review, recorded in `docs/ops/MT-MACHINE-MIGRATION-REVIEW-NOTES.md` and `docs/ops/MT-1.1.4-UPGRADE-TOOLING-REVIEW-NOTES.md`; the operator runbooks are `docs/ops/MT-MACHINE-MIGRATION.md` and `docs/proposals/MT-1.1.0-TO-1.1.4-UPGRADE.md`.

#### Machine Migration — `export-tenant.sh` + `import-tenant.sh`

Move one tenant to a **fresh** host. `export-tenant.sh <tenant>` runs on the source and is self-contained (it needs only the container runtime + `jq` + `tar`, so a v1.1.0 source works); it produces a single `0600` tar bundle: `meta.txt`, a `pg_dump -Fc` `db.dump` (verified for the `PGDMP` magic header), `manifest-lunarwing.env` / `manifest-bridge.env` carry-over keys, and `state.tar.gz` (OMEMO store + workspace, excluding `*.sock`, `state/config.toml`, and `*.wasm`). `import-tenant.sh <bundle.tar>` runs on the target and **delegates all init/runtime specifics to `lunarwing-mt-admin.sh`** — so the target's init system and container mode need not match the source — via `add-tenant --no-health` → `build-tenant --with-wasm` → inject carried secrets → `restore-tenant --yes` → restore state → `install-wasm` → optional `--start`.
...
- **PostgreSQL only.** A non-`postgres` `DATABASE_BACKEND` is refused on both sides rather than risk a silent empty import.

> **Status:** Implemented; **source-side export is live-validated** on a real throwaway `testbot` tenant (consistent quiesced OMEMO snapshot, loopback `LLM_BASE_URL` correctly not carried, `0600` bundle, role/db parsed from `DATABASE_URL`) per `docs/ops/IMPORT_EXPORT_EXAMPLE.txt`. The full **import + cutover end-to-end** — including OMEMO decryption and `SECRETS_MASTER_KEY`-decryptable secret continuity on the new host — is **pending live e2e** (the example log stops at "import-ready"). See *Known Issues*.
...
> **Status:** Implemented; two adversarial passes recorded (`285a5b7d`, `e3ce73ab`; guard `95b0a193`), all residuals fixed in `docs/ops/MT-1.1.4-UPGRADE-TOOLING-REVIEW-NOTES.md`. The guard and `--dry-run` paths are reasoned-through, but a **full live v1.1.0 → v1.1.4 `rootless-adopt` run is not yet evidenced** — treat the end-to-end path as adversarially-reviewed but **UNTESTED live**. The script header also notes that v1.1.0 → v1.1.5 targets need further refactoring and that a separate v1.0.3 → v1.1.0-era harness is still TODO.

#### Generic Same-Host Version Upgrade — `upgrade-tenant-version.sh`

`upgrade-tenant-version.sh <tenant>` is the **rootful, same-Docker-host** version-bump tool (default `--target v1.1.2`), generalizing the one-off `upgrade-tenant-kageho.sh`. It is explicitly *not* the rootless-flip tool — it stays rootful for 1.0.x / 1.1.x → 1.1.x jumps where the only schema delta is additive (the reflex tables V19/V20 + the self-heal dedup V21). It **defaults to a read-only dry-run** (changes nothing until `--apply`) and gates on PostgreSQL ≥ 15 (V21 uses `UNIQUE NULLS NOT DISTINCT`), pending-migration / duplicate-`memory_documents` detection, a clean working tree, and a WeeChat pre-flight. Lessons carried forward from the live kageho run: `render-units` before start (pre-1.1.0 tenants carry the old `weechat-<t>` unit name), and restart the WeeChat adapter **last** to avoid the relay-readiness 401 race (verifying `ws_connected`).

> **Status:** Implemented. The predecessor `upgrade-tenant-kageho.sh` was **run live** for the kageho 1.0.x → 1.1.2 upgrade (2026-06-18); the generalized `upgrade-tenant-version.sh` is a basic implementation carrying those lessons forward. Its dry-run gates are read-only and safe, but a live `--apply` run of the generalized form is not separately evidenced here.
...
> **Status:** `upgrade-preflight.sh` is Implemented (read-only; review pass 2 made the `CAUTION` verdict reachable and added the row-count / `pg_isready` gating). `rehearse-testbot.sh` is **explicitly UNTESTED** — its own banner reads *"UNTESTED HELPER (generated, NOT yet validated — review before running)"*; it is syntax/shellcheck-validated only, and its marker proves a **DB round-trip only**, not OMEMO or encrypted-secret continuity. See *Known Issues*.
...
> **Status:** Implemented; the down-tenant refusal and argv-token rejection are enforced in-script. Live verification of the installed `lunarwing-mt-health.timer` / cron schedule on a real fleet is not separately evidenced in this delta.
...
- **Per-area passes (before → after, as stated in the commit messages):** `architecture/` 19 → 6 (kept the genuine specs; archived 8 session-logs/fix-notes; deleted 5 scratch stubs); `guides/` 56 → 23 (archived 19 vendored + 6 deprecated; deleted 8 stubs/duplicates); `internal/` folded 48 vendored and archived 23 stubs, keeping 5 active docs; `proposals/` 61 → 32 (archived 29 shipped/superseded — including the entire `VisionProject/`, now that the OCR/vision service has shipped — with **zero** hard deletions); `ops/` a light pass that archived 3 shipped checklists (`GOALS_1.1.2*`, `GOALS_1.1.3`) to the existing `ops/history/`; `reference/` pruned to genuine LunarWing references; `releases/` all 7 release notes **kept immutable** (audited only).
...
- **`docs/DOCS_AUDIT.md` reconciled** against the current tree (6 more items closed; 11 remain, all but two being IronClaw → LunarWing rename fixes in files **outside** `docs/`), and **`docs/README.md` rebuilt from scratch** as a complete, fully-linked index of the reorganized tree.
- **Over-archiving of `internal/` walked back (follow-up).** The initial `internal/` pass was judged too aggressive; a follow-up restored 3 codex-worker design notes and consolidated 8 scattered `custom_*` source-repo bookmarks into a single new **`docs/internal/COMPONENT_SOURCES.md`**.
...
- **WeeChat WSS relay could silently half-open.** The relay WebSocket in `ws_adapter.py` connected with `heartbeat=None`, so a silent / half-open drop went undetected. It now sets `heartbeat=25` to surface dead connections (`356b31ec`).
...
- **No tooling existed for the in-place upgrade of a > v1.1.0 tenant** (the v1.1.4 `GOALS_1.1.4.md` #10 gap). v1.1.5 ships the `upgrade-tenant.sh` rootless-adopt path, the `start_tenant_postgres` data-orphan guard, `upgrade-preflight.sh`, and `enable-health-fleet.sh`. **Note:** this closes the *tooling-existence* gap, **not** the live-validation gap — see *New in v1.1.5* below.
- **No QA'd alternative to the risky rootful → rootless flip existed.** v1.1.5 ships the clean-host `export-tenant.sh` / `import-tenant.sh` path (which routes through the QA'd `add-tenant` flow and keeps the old host as rollback), and fixes the real-use `GATEWAY_HOST` / `HTTP_HOST` bind-reset found while using it.

### New in v1.1.5 (migration/upgrade tooling caveats)

- **`ic/scripts/rehearse-testbot.sh` is UNTESTED.** The throwaway-tenant migration-rehearsal helper is generated and syntax/shellcheck-validated only — **never executed** — and is banner-marked accordingly in both the script header and `docs/ops/MT-MACHINE-MIGRATION.md`. Its seeded marker proves a **DB round-trip only**; it does not exercise OMEMO or encrypted-secret (`SECRETS_MASTER_KEY`) continuity. For a real rehearsal, send a live message + an OMEMO chat through the testbot and confirm both survive on the new host.
- **Live cross-machine migration has not been validated end-to-end.** `export-tenant.sh` / `import-tenant.sh` are adversarially-reviewed and a real-use bind-address regression was fixed, but a full `export → import → start` cutover with verified OMEMO device-store + encrypted-secret-row continuity on a second host has **not** been run live. `--dry-run` validates the *plan*, not the outcome; dump validation is a `PGDMP`-header + non-zero-size check, not a full integrity verify.
- **In-place v1.1.0 → v1.1.4 upgrade is dry-run / adversarial-review validated, not live-validated on a real > v1.1.0 tenant.** `upgrade-tenant.sh` / `upgrade-preflight.sh` passed two adversarial passes, but the in-place upgrade of an actual production tenant past v1.1.0 remains open (`docs/proposals/MT-1.1.0-TO-1.1.4-UPGRADE.md` is pending operator go-ahead; `GOALS_1.1.4.md` #10 still unchecked). Treat the first tenant as a canary with full backups.
- **Machine migration and in-place upgrade are PostgreSQL-only by design.** Both `export-tenant.sh` / `import-tenant.sh` **refuse a libSQL tenant** rather than risk a silent empty import; a libSQL DB file must be migrated by hand. Intentional limitation, newly introduced with the tooling.
...
- **A few non-critical cargo tests fail** (tracked in `docs/proposals/CARGO_TESTS_FIX.md`), and the `e2e_advanced_traces` bootstrap-greeting tests remain among the pre-existing, env-dependent e2e failures (2 failures). See `docs/bugs/BUG-e2e-bootstrap-greeting-tests.md`. Not addressed by v1.1.5's bug-fix scope.
...
- **Multica bridge remains pre-release/experimental.** May require significant improvements; refinements are scheduled for a later feature release (see *Deferred*).
- **DarkIRC WASM channel and adapter are not fully multi-tenant-aware.** The v1.1.3 work namespaced the channel's state under `state/<tenant_id>/…`, but the adapter side and live multi-tenant validation remain outstanding; MT setups do not yet "just work" with DarkIRC.
...
2. **Crate version bump to 1.1.5 is pending.** Manifests currently report `1.1.4`; rebuild from the tag once the bump lands.
3. **Optional new env var: `LLM_TURN_BUDGET_SECS` (default `270`).** Caps the total wall-clock time of one logical LLM call (all retries included) and must stay **below** the `handle_message` turn timeout (`AGENT_HANDLE_MESSAGE_TIMEOUT_SECS`, default `300`). Set `0` to disable. No action is required to adopt the default.
4. **Multi-tenant operators — read the runbooks and preflight first.** Before any in-place upgrade, run `ic/scripts/upgrade-preflight.sh --all` and act on any `STOP`/`CAUTION` verdict (its key check is the rootful → rootless data-orphan hazard). The in-place upgrade (`upgrade-tenant.sh`) and machine migration (`export-tenant.sh` / `import-tenant.sh`) are **adversarially-reviewed but not yet live-validated end-to-end** — treat the first tenant as a canary, keep full backups, and prefer rehearsing on a throwaway tenant. See `docs/ops/MT-MACHINE-MIGRATION.md` and `docs/proposals/MT-1.1.0-TO-1.1.4-UPGRADE.md`.
5. **The rootful → rootless PostgreSQL flip is guarded.** A plain start that would create a fresh empty rootless PG while a legacy root-store DB still holds the data is now refused by `start_tenant_postgres`. Resolve it deliberately: migrate with `upgrade-tenant.sh`, keep rootful with `LUNARWING_MT_ROOTLESS=false`, or acknowledge an intended fresh DB with `LUNARWING_MT_ACK_ROOTLESS_FLIP=<tenant>`.
6. **Machine migration is PostgreSQL-only and a downtime cutover.** libSQL tenants are refused by the tooling; the tenant is down from export until it is started on the new host. Plan a maintenance window, and keep the old host as the rollback until the new one is verified.
7. **Enabling fleet-wide self-heal on an existing fleet.** Use `ic/scripts/enable-health-fleet.sh` (it refuses to arm the timer while a tenant daemon is down unless `--allow-down`). Put the Gotify URL/token in `/etc/lunarwing/health.env` (mode `0600`) via `--gotify-token-file` or `LUNARWING_MT_GOTIFY_TOKEN` — never on the command line.
...
| Feature | Target |
|---------|--------|
| Live validation of the migration + in-place upgrade tooling (run `rehearse-testbot.sh`, verify OMEMO/secret continuity, canary in-place upgrade of a > v1.1.0 tenant) | v1.1.6 |
...
*Still open at the time these notes were drafted: the crate-version bump to 1.1.5 + `cargo test` sweep, a live end-to-end run of the machine-migration route (`export → import → start` with OMEMO/secret continuity), a live in-place v1.1.0 → v1.1.4 upgrade of a canary tenant, and the broader automated `release-test.sh` sweep. The release date is **TBD** until these conclude.*
