...
> **Status:** `upgrade-preflight.sh` is Implemented (read-only; review pass 2 made the `CAUTION` verdict reachable and added the row-count / `pg_isready` gating). `rehearse-testbot.sh` carries its own **UNTESTED-helper banner** (*"generated, NOT yet validated — review before running"*); it **was** run source-side — it created the throwaway testbot and seeded the `migration_rehearsal_marker` that the export then bundled (`docs/ops/IMPORT_EXPORT_EXAMPLE.txt`: *"Source-side rehearsal: fully validated"*) — but the full export → import → verify round-trip has not been completed, and even then the marker would prove a **DB round-trip only**, not OMEMO or encrypted-secret continuity. See *Known Issues*.
...
- **No general tooling existed for the in-place upgrade of a live tenant on a version > v1.1.0** (only one-off hardcoded scripts). v1.1.5 ships the generalized `upgrade-tenant.sh` rootless-adopt path, the `start_tenant_postgres` data-orphan guard, `upgrade-preflight.sh`, and `enable-health-fleet.sh`. **Note:** this closes the *tooling* gap, **not** the live-validation gap (`GOALS_1.1.4.md` #11 remains unchecked) — see *New in v1.1.5* below.
...
- **`ic/scripts/rehearse-testbot.sh` round-trip is not yet complete.** The throwaway-tenant migration-rehearsal helper carries an UNTESTED-helper banner (in both the script header and `docs/ops/MT-MACHINE-MIGRATION.md`), but it **was** run source-side — it created the testbot and seeded the `migration_rehearsal_marker` that the export bundled (`docs/ops/IMPORT_EXPORT_EXAMPLE.txt`). What remains unrun is the **import side**: the full export → import → verify round-trip on a second host. Even once completed, the marker proves a **DB round-trip only** — it does not exercise OMEMO or encrypted-secret (`SECRETS_MASTER_KEY`) continuity, so a real rehearsal should also send a live message + an OMEMO chat through the testbot and confirm both survive on the new host.
...
- **In-place v1.1.0 → v1.1.4 upgrade is dry-run / adversarial-review validated, not live-validated on a real > v1.1.0 tenant.** `upgrade-tenant.sh` / `upgrade-preflight.sh` passed two adversarial passes, but the in-place upgrade of an actual production tenant past v1.1.0 remains open (`docs/proposals/MT-1.1.0-TO-1.1.4-UPGRADE.md` is pending operator go-ahead; `GOALS_1.1.4.md` #11 still unchecked). Treat the first tenant as a canary with full backups.
...
| Feature | Target |
|---------|--------|
| Live validation of the migration + in-place upgrade tooling (complete the `rehearse-testbot.sh` export → import round-trip, verify OMEMO/secret continuity, canary in-place upgrade of a > v1.1.0 tenant) | v1.1.6 |
