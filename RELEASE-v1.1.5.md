...
- **`ic/scripts/rehearse-testbot.sh` (the throwaway-tenant rehearsal helper) is not fully exercised.** It carries an UNTESTED-helper banner and was run source-side only — it created the testbot and seeded the `migration_rehearsal_marker` the export bundled (`docs/ops/IMPORT_EXPORT_EXAMPLE.txt`) — but its import-side round-trip was never completed. This is now low-risk in practice, since the migration path it rehearses has been live-validated on a production tenant (above); still, treat the helper itself as unproven until run end-to-end, and note its marker would prove a **DB round-trip only**, not OMEMO or encrypted-secret (`SECRETS_MASTER_KEY`) continuity.
...
4. **Multi-tenant operators — read the runbooks and preflight first.** The **machine-migration path (`export-tenant.sh` / `import-tenant.sh`) has been live-validated on a production tenant**; the **in-place upgrade (`upgrade-tenant.sh`) is adversarially-reviewed but not yet live-validated end-to-end**. Before any in-place upgrade, run `ic/scripts/upgrade-preflight.sh --all` and act on any `STOP`/`CAUTION` verdict (its key check is the rootful → rootless data-orphan hazard), treat the first tenant as a canary, and keep full backups. See `docs/ops/MT-MACHINE-MIGRATION.md` and `docs/proposals/MT-1.1.0-TO-1.1.4-UPGRADE.md`.
...
| Feature | Target |
|---------|--------|
| Remaining migration/upgrade validation (spot-check OMEMO/secret continuity on a migration; canary in-place upgrade of a > v1.1.0 tenant) | v1.1.6 |
...
*Completed: a live end-to-end `export → import → start` cutover of a production tenant via the machine-migration route (OMEMO/secret continuity not separately spot-checked during that run). Still open at the time these notes were drafted: the crate-version bump to 1.1.5 + `cargo test` sweep, a live in-place v1.1.0 → v1.1.4 upgrade of a canary tenant, and the broader automated `release-test.sh` sweep. The release date is **TBD** until these conclude.*
