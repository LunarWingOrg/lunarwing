...
Bridge-backed channels can keep their protocol client in a separate local
helper binary. In that layout:

```text
channels-src/xmpp/           # installable WASM channel
...
```json
{
  ...
  "setup": {
    ...
    "required_fields": [
      {
        "name": "bridge_url",
        "prompt": "Local bridge URL",
        "optional": false
      },
      {
        "name": "room_list",
        "prompt": "Allowed rooms (comma-separated)",
        "optional": true,
...
`required_fields` are persisted as extension settings under
`extensions.<channel>.setup_fields`. Use them for non-secret configuration
such as local bridge URLs, policy flags, room lists, or device IDs. Use
`required_secrets` only for sensitive values that must go through the encrypted
secrets store.
...
```bash
# Build the installable XMPP channel
...
./bridges/xmpp-bridge/target/release/xmpp-bridge
```

Then configure the installable `xmpp` channel through onboarding or the
extensions/channel setup UI. The channel stores `xmpp_password` and the
optional `xmpp_bridge_token` as secrets, and stores `xmpp_jid`, `bridge_url`,
and policy fields as setup fields.
