...
# LLM_BASE_URL (the local proxy) is carried ONLY if it's a non-local custom endpoint.
#
...
LW_KEYS=(SECRETS_MASTER_KEY XMPP_JID XMPP_PASSWORD XMPP_DM_POLICY XMPP_ALLOW_FROM
         ...
         XMPP_OMEMO_DEVICE_ID LLM_API_KEY LLM_MODEL)   # LLM_BASE_URL handled specially below
...
if [[ -d "$STATE_DIR" ]]; then
  if $DRY_RUN; then
    ...
    [[ -d "$STATE_DIR/xmpp" ]] && note "  includes OMEMO store state/xmpp" || note "  (no OMEMO store present yet)"
  else
