...
async def test_oauth_install_gmail(lunarwing_server):
...
async def test_oauth_configure_returns_auth_url(lunarwing_server):
...
async def test_oauth_activate_returns_auth_url(lunarwing_server):
    """Activate on un-authenticated gmail returns auth_url."""
    if not _gmail_installed:
        pytest.skip("gmail not installed")

    r = await api_post(
        lunarwing_server, "/api/extensions/gmail/activate", timeout=30
    )
    assert r.status_code == 200
    data = r.json()
...
async def test_oauth_callback_exchanges_token(lunarwing_server):
    ...
    async with httpx.AsyncClient() as client:
        r = await client.get(
            f"{lunarwing_server}/oauth/callback",
...
async def test_oauth_callback_replay_rejected(lunarwing_server):
...
async def test_oauth_callback_invalid_state(lunarwing_server):
...
async def test_oauth_extension_authenticated(lunarwing_server):
...
async def test_oauth_tools_registered(lunarwing_server):
...
async def test_remove_during_pending_oauth_invalidates_callback(lunarwing_server):
    ...
    remove_r = await api_post(
        lunarwing_server, "/api/extensions/gmail/remove", timeout=30
    )
...
async def test_cleanup_gmail(lunarwing_server):
