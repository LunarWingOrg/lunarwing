...
async def _get_extension(lunarwing_server, name):
...
@pytest.fixture
async def installed_gmail(lunarwing_server):
    ...
    if ext:
        r = await api_post(lunarwing_server, "/api/extensions/gmail/remove", timeout=30)
...
@pytest.fixture
async def auth_url(lunarwing_server, installed_gmail):
...
async def test_oauth_url_state_is_unique(lunarwing_server, installed_gmail, oauth_params, auth_url):
