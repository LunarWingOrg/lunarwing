...
The listener is **ephemeral** — it is started only when an OAuth flow is initiated (e.g., `lunarwing tool auth <name>`) and shut down after the callback is received or the timeout expires.
