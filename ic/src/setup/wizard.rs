...
fn is_allowed_extension_setup_setting_path(name: &str, setting_path: &str) -> bool {
    let namespaced_prefix = format!("extensions.{name}.");
    setting_path.starts_with(&namespaced_prefix)
        || ALLOWED_GLOBAL_EXTENSION_SETUP_SETTING_PATHS.contains(&setting_path)
}

fn validate_extension_setup_setting_path(name: &str, setting_path: &str) -> Result<(), SetupError> {
    if is_allowed_extension_setup_setting_path(name, setting_path) {
        return Ok(());
    }
...
impl SetupWizard {
            ...
            if let Some(ref ctx) = secrets {
                let result = if let Some(cap_file) = discovered_by_name.get(&channel_name) {
                    if !cap_file.setup.required_secrets.is_empty()
                        || !cap_file.setup.required_fields.is_empty()
                    {
                ...
                if result.enabled {
                    if let Some(cap_file) = discovered_by_name.get(&channel_name) {
                        self.persist_wasm_channel_setup_fields(
                            &channel_name,
                            &cap_file.setup,
                            &result.setup_fields,
                        )
                        ...
                        if result.restart_required {
                            print_info(&format!(
                                "{} has setup changes that require a restart to fully apply.",
                                channel_name
                            ));
                        }
                    }
    ...
    async fn persist_wasm_channel_setup_fields(
        &self,
        channel_name: &str,
        ...
        let value = serde_json::to_value(fields)
            .map_err(|e| SetupError::Config(format!("Failed to encode setup fields: {e}")))?;
        ...
        if let Some(ref pool) = self.db_pool {
            ...
            store
                ...
                .map_err(|e| {
                    SetupError::Database(format!(
                        "Failed to save setup fields for '{}': {}",
                        channel_name, e
                    ))
                })?;
            ...
            for field in &setup.required_fields {
                if let Some(setting_path) = field.setting_path.as_deref()
                    && let Some(field_value) = fields.get(&field.name)
                {
                    validate_extension_setup_setting_path(channel_name, setting_path)?;
                    store
                        .set_setting(
                            self.owner_id(),
                            setting_path,
        ...
        Err(SetupError::Database(format!(
            "No database connection available to persist setup fields for '{}'",
            channel_name
        )))
    }
