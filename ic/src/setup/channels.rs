...
pub async fn setup_tunnel(settings: &Settings) -> Result<TunnelSettings, ChannelSetupError> {
    ...
    print_info("  - Slack, GitHub webhooks");
    println!();
...
pub struct WasmChannelSetupResult {
    ...
    pub setup_fields: std::collections::HashMap<String, String>,
...
/// prompts the user for each required secret and field.
pub async fn setup_wasm_channel(
    ...
    let mut setup_fields = std::collections::HashMap::new();
    ...
    for field in &setup.required_fields {
        let value = match (field.optional, field.input_type) {
            (true, crate::tools::wasm::ToolSetupFieldInputType::Password) => {
                optional_input(&field.prompt, Some("leave empty to skip"))?
            }
            (true, crate::tools::wasm::ToolSetupFieldInputType::Text) => {
                optional_input(&field.prompt, Some("leave empty to skip"))?
            }
            (false, crate::tools::wasm::ToolSetupFieldInputType::Password) => {
                let value = secret_input(&field.prompt)?;
                Some(value.expose_secret().to_string())
            }
            (false, crate::tools::wasm::ToolSetupFieldInputType::Text) => {
                Some(input(&field.prompt)?)
            }
        };
        ...
        setup_fields.insert(field.name.clone(), trimmed.to_string());
        print_success(&format!("{} configured", field.name));
    }
...
mod tests {
    ...
    fn test_http_webhook_secret_hint_reflects_current_behavior() {
        ...
        assert!(!hint.contains("lunarwing secret get"));
    }
}
