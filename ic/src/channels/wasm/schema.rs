...
use crate::tools::wasm::{
    CapabilitiesFile as ToolCapabilitiesFile, RateLimitSchema, ToolFieldSetupSchema,
};
...
impl ChannelCapabilitiesFile {
    ...
    pub fn validate(&self) {
        ...
        if (!self.setup.required_secrets.is_empty() || !self.setup.required_fields.is_empty())
            && self.setup.setup_url.is_none()
        {
            tracing::warn!(
                channel = self.name,
                "setup requirements defined but no setup.setup_url — \
...
pub struct SetupSchema {
    ...
    /// Non-secret setup fields persisted with the extension configuration.
    #[serde(default)]
