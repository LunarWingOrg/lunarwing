...
impl WasmChannel {
    ...
    fn add_host_functions(linker: &mut Linker<ChannelStoreData>) -> Result<(), WasmChannelError> {
        ...
        lunarwing::agent::channel_host::add_to_linker::<
            _,
...
fn status_to_wit(
    ...
    Some(match status {
        ...
        // Suggestions, turn cost, and skill activations are web-gateway-only; skip for WASM channels
...
mod tests {
    ...
    fn xmpp_wasm_path() -> std::path::PathBuf {
        ...
        let target =
            manifest_dir.join("channels-src/xmpp/target/wasm32-wasip2/release/xmpp_channel.wasm");
        if target.exists() {
            return target;
        }
    ...
    async fn create_real_xmpp_channel(config_json: &str) -> WasmChannel {
        ...
        let wasm_bytes = std::fs::read(xmpp_wasm_path()).expect("Failed to read XMPP WASM module");
        let prepared = runtime
            .prepare("xmpp", &wasm_bytes, None, Some("XMPP".to_string()))
            .await
            .expect("Failed to prepare XMPP WASM module");

        WasmChannel::new(
            runtime,
            prepared,
            ChannelCapabilities::for_channel("xmpp"),
            "default",
    ...
    #[tokio::test]
    async fn test_real_xmpp_respond_can_read_runtime_config_after_start() {
        require_xmpp_wasm!();

        let channel = create_real_xmpp_channel(
            &serde_json::json!({
                "display_name": "XMPP",
        ...
        let msg = crate::channels::IncomingMessage::new("xmpp", "tester@example.com", "hello")
            .with_metadata(serde_json::json!({
                "xmpp_target": "tester@example.com",
                "xmpp_type": "chat"
            }));

        let err = channel
            .respond(&msg, crate::channels::OutgoingResponse::text("reply"))
            .await
            .expect_err("respond should fail without a reachable bridge");
        assert!(
            !err.to_string()
                .contains("xmpp runtime config is unavailable"),
            "respond lost persisted runtime config: {err}"
        );
    ...
    #[tokio::test]
    async fn test_real_xmpp_broadcast_can_read_runtime_config_after_start() {
        require_xmpp_wasm!();
        ...
        let err = channel
            .broadcast(
                "tester@example.com",
                crate::channels::OutgoingResponse::text("proactive reply"),
            )
            .await
            .expect_err("broadcast should fail without a reachable bridge");
        assert!(
            !err.to_string()
                .contains("xmpp runtime config is unavailable"),
            "broadcast lost persisted runtime config: {err}"
        );
