...
pub struct IncomingMessage {
    ...
    pub user_id: String,
    /// Stable instance owner scope for this LunarWing deployment.
    pub owner_id: String,
...
pub fn routing_target_from_metadata(metadata: &serde_json::Value) -> Option<String> {
    metadata
        ...
        .or_else(|| {
            metadata.get("xmpp_target").and_then(|value| match value {
                serde_json::Value::String(s) => Some(s.clone()),
                _ => None,
            })
        })
        .or_else(|| {
            metadata.get("xmpp_room").and_then(|value| match value {
                serde_json::Value::String(s) => Some(s.clone()),
                _ => None,
            })
        })
...
mod tests {
    ...
    #[test]
    fn routing_target_from_metadata_supports_xmpp() {
        let metadata = serde_json::json!({
            "xmpp_target": "room@conference.example",
            "xmpp_room": "room@conference.example",
        });
        assert_eq!(
            routing_target_from_metadata(&metadata).as_deref(),
            Some("room@conference.example")
        );
    }
}
