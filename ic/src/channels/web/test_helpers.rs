...
use crate::channels::web::auth::{CombinedAuthState, MultiAuthState};
...
impl TestGatewayBuilder {
    ...
    pub async fn start(
        ...
        let auth = CombinedAuthState::from(MultiAuthState::single(
    ...
    pub async fn start_multi(
        ...
        let addr: SocketAddr = "127.0.0.1:0"
            .parse()
            .expect("hard-coded address must parse"); // safety: constant literal
        let bound = start_server(addr, state.clone(), CombinedAuthState::from(auth)).await?;
        Ok((bound, state))
    }
}
