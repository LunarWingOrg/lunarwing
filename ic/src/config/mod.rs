...
impl Config {
    ...
    pub async fn from_db_with_toml(
        ...
        crate::bootstrap::load_lunarwing_env();
    ...
    ///
    /// Loads both `./.env` (standard, higher priority) and `~/.lunarwing/.env`
    ...
    /// If `None`, tries the default path `~/.lunarwing/config.toml` (missing
