...
pub struct Cli {
    ...
    ///
    /// Skips interactive approval prompts for standard tools. Other safeguards
...
/// Run the REPL CLI subcommand (connects to a running daemon).
#[cfg(unix)]
pub async fn run_repl_command(repl_cmd: &ReplCommand) -> anyhow::Result<()> {
    repl_cmd.run().await
}
