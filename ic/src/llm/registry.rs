...
//! ```text
//!   ┌─────────────────────┐    ┌──────────────────────────┐
//!   │  providers.json     │    │ ~/.lunarwing/providers.json│
...
impl ProviderRegistry {
    ...
    ///
    /// User providers from `~/.lunarwing/providers.json` are appended,
...
fn user_providers_path() -> Option<std::path::PathBuf> {
    Some(crate::bootstrap::lunarwing_base_dir().join("providers.json"))
}
