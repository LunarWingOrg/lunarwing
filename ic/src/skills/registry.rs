...
//! 2. User skills directory (`~/.lunarwing/skills/`) -- Trusted
//!
...
pub struct SkillRegistry {
    ...
    /// User skills directory (~/.lunarwing/skills/). Skills here are Trusted.
    user_dir: PathBuf,
    /// Registry-installed skills directory (~/.lunarwing/installed_skills/). Skills here are Installed.
    installed_dir: Option<PathBuf>,
    /// Optional workspace skills directory.
    workspace_dir: Option<PathBuf>,
    /// Bundled skill (name, SKILL.md content) pairs compiled into the binary.
...
impl SkillRegistry {
    ...
    ///
    /// Bundled skills are the lowest-priority discovery source (overridden by
    /// workspace, user, and installed skills). They are loaded with `Trusted`
    /// trust level since they ship with the application.
    pub fn with_bundled_content(mut self, content: &'static [(String, String)]) -> Self {
        self.bundled_content = content;
        self
    }

    /// Set the maximum recursion depth when scanning skill directories.
    ///
    /// Subdirectories without `SKILL.md` are recursed into up to this depth
    /// to support bundle layouts nested within category directories.
    ...
    pub async fn discover_all(&mut self) -> Vec<String> {
        ...
        // 4. Bundled skills (lowest priority — overridden by all filesystem skills)
        if !self.bundled_content.is_empty() {
            let bundled = load_bundled_skills(self.bundled_content, &seen).await;
...
/// Load bundled skills from in-memory (name, content) pairs, skipping names already seen.
async fn load_bundled_skills(
    bundled: &'static [(String, String)],
    seen: &HashSet<String>,
) -> Vec<(String, LoadedSkill)> {
    let mut results = Vec::new();
...
/// Load and validate a skill from raw string content (no file I/O).
