...
PostgreSQL schema: `migrations/V1__initial.sql` through `V18__routine_retry.sql` (managed by `refinery`). V1 is the base schema; later migrations add tables, columns, rename `claude_code_events` → `job_events`, and extend routines with retry policy fields.
...
DATABASE_BACKEND=libsql LIBSQL_PATH=~/.lunarwing/test.db cargo run
