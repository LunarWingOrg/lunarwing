...
async fn async_main() -> anyhow::Result<()> {
    ...
    match &cli.command {
        ...
        Some(Command::Login { openai_codex: _ }) => {
            init_cli_tracing();
            println!("The login command no longer has any active providers to authenticate with.");
            return Ok(());
        }
    ...
    tracing::debug!("Starting LunarWing...");
    ...
    if let Some(ref ssh_bridge) = components.ssh_bridge
        && let Some(ref secrets_store) = components.secrets_store
    {
