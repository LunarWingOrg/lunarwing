...
3. Store the token:
   ...
   ```
   lunarwing secret set github_token YOUR_TOKEN
   ```
...
```text
GitHub token not found in secret store. Set it with: lunarwing secret set github_token <token>...
```
...
- Update the token: `lunarwing secret set github_token NEW_TOKEN`.
