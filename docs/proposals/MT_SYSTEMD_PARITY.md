...
these 7 items remain:*

1. **Delete `docs/proposals/NANOCODE_WORKER_SECRETS.md`.** Both of its ideas —
   secret injection into nanocode workers and `nanocode_wss` per-tenant port
   allocation — are implemented in `lunarwing-mt-admin.sh`; the file itself now
   says "Both already implemented now".
2. **`docs/proposals/ROOTLESS_WORKER_OPENRC_UNITS.md:73`** — residual false claim
...
3. **`docs/proposals/IC_REPAIR_FOLLOWUPS.md` item #2** (systemd `.timer/.service`
...
4. **`docs/proposals/ROOTLESS_DEFAULT_INIT_GATING.md`** — add a **SUPERSEDED**
   banner. The proposed rootful init-gate was intentionally *not* built; the gap
   was instead closed by full systemd-rootless Quadlet support (this doc). Do not
   mark it "implemented as proposed".
5. **`docs/ops/MULTITENANCY-PRODUCTION.md`** unit counts — L309 still says "3
   user-level systemd units" and L332 "3 init scripts". Actual is **5** systemd
   user units (proxy, weechat, weechat-adapter, xmpp-bridge, main daemon) **plus**
   the pg/worker Quadlet/init units. Update both tables.
6. **`docs/proposals/WORKER_HEALTHCHECK_PORT_MISMATCH.md`** — line-anchor drift
   (content correct): `mt-admin.sh` refs `1145→1612`, `1239→1735` (symptom refs
   `→1701/1813`). Source/code anchors are already exact.
7. **`docs/proposals/IC_REPAIR_FOLLOWUPS.md`** — line-anchor drift: `self-heal.sh`
