# Release Notes for LunarWing v1.1.8 — Codename `Unknown`

**Release Date:** 2026-07-XX *(to be set before tag — see GOALS_1.1.8 #29)*

> Codename *Unknown* — placeholder for the final codename. v1.1.8 is a **feature + polish** release that breaks the pure-polish posture of odd-numbered cycles with three deliberate exceptions: a brand-new **opencode external worker**, the completion of the **Agent SSH tooling** (delivery Options 2 and 3 plus hardening), and the **hard-disable of DarkIRC** unless explicitly opted in. Beneath those headline changes, the release folds in a round of multi-tenant admin streamlining (SSH provisioning folded into `start-tenant`, persistent runtime detection, `add-tenant`/`add-tenants` enhancement flags), the removal of the default GitHub WASM tool and bundled MCP manifests from the fresh-install registry, routine-engine improvements carried over from v1.1.7, and a documentation sweep.

## Overview

Per the release cadence (`docs/ops/RELEASE_CADENCE.md`), odd-numbered releases focus on bug fixes, security improvements, and polishing. v1.1.8 is **mostly** polish, but introduces three new features:

1. **The opencode external worker** (`opencode4lunarwing/`) — a persistent [opencode](https://opencode.ai) (sst/opencode) worker container speaking the `ironclaw-agent-v1` WebSocket protocol, with the `@opencode-ai/sdk`, optional Paseo MCP integration, git/ssh key support, and health endpoints. It is the third external worker container (joining nanocode and pebble). The daemon-side `ExternalWorkerManager` was already generic, so **no Rust changes were required** to add it — only the container, the port-registry v10→v11 migration (`opencode_wss` / `opencode_health`), and the full `lunarwing-mt-admin.sh` lifecycle (`build-opencode-worker`, `configure-opencode`, start/stop/doctor/status, `--with-opencode`, `--opencode-model`/`--opencode-base-url`).
...
3. **DarkIRC is now hard-disabled by default.** Previously the disabled flag was advisory — services could still be rendered/built. v1.1.8 makes the disable fail-closed: DarkIRC services are **not created** unless the `--enable-darkirc` flag is set on `add-tenant`/`start-tenant` **and** `build-darkirc` is invoked. See *DarkIRC hard-disable*.

Additional work includes removing the default GitHub WASM tool and the bundled MCP server manifests from the fresh-install registry catalog, threading `--llm-model` / `--gateway-host` / `--xmpp-allow-from` through `add-tenant` / `add-tenants`, persisting the detected container-runtime choice so `LUNARWING_CONTAINER_RUNTIME` only needs to be set once, self-heal pipeline fixes for opencode worker discovery, and an extensive documentation pass.
...
- **`ssh_git` first-connect pin persistence is not built.** The chosen design is `Strict` + pinned `known_host_key` (option B). Actual first-connect pin persistence remains an option for a future cycle.
- **No full `ssh_git` clone data-transfer test.** Only an empty-repo transport proof exists; a seeded-repo clone test is outstanding.
...
- **Legacy same-host upgrade path and kawarimi owner-scope automation are not yet live-validated.** The legacy upgrade harness is hardened but not exercised end-to-end on a production tenant; the kawarimi cross-machine migration's automated import-side owner-scope rekey (Phase 1) remains proposed/deferred. Manual measure→rekey→verify is the only supported path today. See `docs/proposals/MT-LEGACY-UPGRADE-VERIFICATION.md` and `docs/proposals/KAWARIMI-OWNER-SCOPE-CONTINUITY.md`.
...
- **No integration test coverage for the v1.1.8 features.** The SSH tools, opencode worker, DarkIRC hard-disable, and routine-engine improvements have **no** integration tests under `ic/tests/`. The opencode worker was live-validated end-to-end on tenant `octest` (see `docs/proposals/DEFERRED-2026-07-02-OPENCODE-EXTERNAL-WORKER.md`); the SSH tools have unit/smoke tests only. This is a coverage gap, not a defect.
- **`chmod 777` on worker workspace dirs breaks tenant isolation.** (`lunarwing-mt-admin.sh`) The current workaround for the rootless-userns workspace-ownership issue (see *New in v1.1.8*). Tracked in `docs/proposals/SESSION-AUDIT-MT-DARKIRC-EWE-2026-06-23.md` (M1).
- **`tokens` command prints bearer tokens in cleartext.** (`lunarwing-mt-admin.sh`, `show_tokens`) No `--reveal` gate. Tracked in `docs/proposals/SESSION-AUDIT-MT-DARKIRC-EWE-2026-06-23.md` (M2).
...
**A brief note about release cadence.** LunarWing abides by a release cadence to organize `feature`- and `polish`-focused releases — odd-numbered releases (like this one) focus on bug fixes, security improvements, and polishing. For details see `docs/ops/RELEASE_CADENCE.md`. Occasionally, exceptions may be made, but the goal is to stay within this paradigm. v1.1.8 makes three deliberate exceptions: the opencode external worker, the completion of the Agent SSH tooling, and the DarkIRC hard-disable — each a contained, security/privacy-aligned change whose introduction could not be cleanly deferred without blocking downstream work.

## Testing

### In accordance with developer guidelines, a testing period precedes each release.

#### The pre-release checklist lives in `docs/ops/GOALS_1.1.8.md`; the full checklist is in `docs/ops/PRE-RELEASE-TESTING.md`; automated coverage is driven by `ic/scripts/release-test.sh` and `docs/guides/TESTING_GUIDE.md`. These documents may also reference other documents as well.
...
#### Coverage gaps (not blocking, but noted)

The v1.1.8 feature surface has **no dedicated integration test coverage** under `ic/tests/`:
- The Agent SSH tools (built-in `ssh`, `ssh_git`, WASM `ssh` guest) have unit/smoke tests only; the WASM guest has an instantiation smoke test (`commit 2b29f158`).
- The DarkIRC hard-disable is validated by the `mt-admin` CLI dispatch smoke check, not by `cargo test`.
- The routine-engine improvements (#1–#3) ship a dedicated live-tenant test script (`ic/scripts/test-routine-improvements.sh`) that self-declares "COMPLETELY UNTESTED" — it was written from API source inspection and has not been executed end-to-end.
- `ic/scripts/release-test.sh` covers gateway HTTP smoke (16 auth + 2 unauth checks) but does not exercise SSH, opencode, or darkirc.

These gaps are tracked for a future cycle; the live validation that did occur (opencode on `octest`) is the strongest signal for the headline feature.
