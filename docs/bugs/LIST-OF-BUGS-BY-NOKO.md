...
1. `create_job` tool spawns a container and a background job monitor via `spawn_job_monitor_with_context`
...
2. Worker calls `POST /worker/{job_id}/complete` → `report_complete`
3. `report_complete` calls `job_manager.complete_job()`
...
## Reproduction Instance — 2026-05-08
- Spawned a `claude_code` job (ID: `96f3e7d1-fb3d-48ed-b5d7-fbae6ef069a2`)
- Job actually completed inside the container
- Main agent reported status as "completed" based on initial API response
- User reports the job was never properly marked as complete — had to cancel it manually
