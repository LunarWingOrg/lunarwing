...
**Also closed (reconciled 2026-06-07):** M15 — the Discord/Slack/Feishu channel+tool sources (and their READMEs) were deleted in v1.1.1, so the stale-README items are moot. M18 — all Cargo.tomls (`ic/` + the four crates) now declare `AGPL-3.0-or-later`, matching the LICENSE file. M21 — `ic/src/bootstrap.rs` comments corrected to state the real default base dir (`~/.ironclaw`, override with `LUNARWING_BASE_DIR`). The lone remaining skill `display_name` "IronClaw Workflow Orchestrator" (M17 follow-on) was also renamed.
